...
"Move north" => {text: "You move towards the sound of the water and find a clear stream cutting through the tree line. Across the stream, you can see wide open fields. Upstream, you see a mountain area, and downstream you see that the water continues flowing through the trees, pas some caves.", location: "river1"},
...
"Look at hoof-prints" => {text: "There are hoof-prints covering almost every inch of the dirt track. The most recent ones appear to be facing left.", location: "track"}
},
...
cart: @options_cart = {"Take food" => {text: "An angry merchant returns with his horse and shoots you for being a thief.
  GAME OVER", location: "end"},
"Wait for merchant" => {text: "A merchant returns with his horse and finds you waiting by his cart. He quickly checks his goods and is pleased to find you haven't taken anything. He offers you a ride to town in exchange for something he can trade.", location: "trade"}
},
trade: @options_trade = {"Check for item to trade" => {text: "You check your inventory", location: "trade"}
},
...
"Keep moving forwards" => {text: "You head further into the cave, your eyes adjusting slightly to the darkness. You reach a fork in the path. You're really thirsty and feel as though you could pass out.", location: "cave_fork_water"}
},
...
def action_output
  if @action == "Check inventory"
    ...
    thirsty
  elsif @action == "Check for item to trade"
    trade
  else
...
def trade
  if @inventory[:fish] == 1
    puts "You offer up the fish you caught earlier."
    puts "Satisfied, the merchant gives you a ride to the nearest village. You're safe.
  YOU WIN"
    @location = "end"
  else
    puts "You have nothing to trade. The merchant leaves and you are stranded as night draws in. Your clothes are wet from crossing the river and offer no warmth as cold permeates the air. You fall victim to hypothermia and die.
    GAME OVER"
    @location = "end"
  end
end
